...
describe Admin::CategoriesController do
  context 'when user is not logged in' do
    ...
    describe 'PUT #update' do
      let(:category) { create(:category) }
      it_should_behave_like 'has to be logged in' do
        before(:each) { put :update, id: category.id, category: attributes_for(:category) }
      end
    end
    ...
    describe 'DELETE #destroy' do
      let(:category) { create(:category) }
      it_should_behave_like 'has to be logged in' do
        before(:each) { delete :destroy, id: category.id }
      end
    end
  end
  ...
  context 'when authorized user is logged in' do
    ...
    describe 'PUT #update' do
      let(:category) { create(:category) }
      context 'with valid params' do
        before(:each) { put :update, id: category.id, category: { title: 'Lorem Ipsum', slug: 'lorem-ipsum' } }
      ...
      context 'with invalid params' do
        before(:each) do
          Category.any_instance.stub(:update).and_return(false)
